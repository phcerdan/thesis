...
where fibers of fixed length are thrown into a simulation box until certain density of junctions/intersections is reached. Other models used similar random generation, but in 3D \cite{astrom_strain_2008}.
...
Lattice models are being used as well \cite{ronceray_fiber_2015, vahabi_elasticity_2016, sharma_strain-controlled_2016}, the end-to-end distances and the contour lengths can be fully controlled, but the connectivity is compromised, the distribution might be too narrow for simple lattices, (honey-comb, triangular), or rely on random dilutions for over-connected lattices (fcc) \cite{sharma_strain-controlled_2016}.
...
In terms of the network structure, this model is surprisingly effective given its simplicity, this is due that under shear, the network is greatly simplified, and long chains or micro-structures aligned \ang{45} with respect the shear direction exert the main response \cite{janmey_negative_2006}.
...
The angle is fixed as well, and can be tweaked with pre-stress.
...
\begin{itemize}
  \item How to we use real 3D images to extract realistic network architectures?
\end{itemize}
